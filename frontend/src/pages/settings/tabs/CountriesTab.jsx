...
function CountriesTab() {
  ...
  const handleSave = async () => {
    if (!formData.name || formData.name.length < 2)
      return setError("Country name must be at least 2 characters!");
    setSaving(true);
    try {
      if (selected) {
        await api.put(`/countries/${selected.countryId}`, formData);
      } else {
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/countries/${selected.countryId}`);
      await fetchCountries();
      ...
      >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-12">
            <MdPublic size={36} style={{ color: "#e2e8f0" }} />
            <p
              style={{
                fontSize: "13px",
                fontWeight: "600",
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              ...
              >
                {selected ? "Edit Country" : "Add Country"}
              </h2>
              <button
